 ...
 * opens the app on the wrong page — nothing inside Quire can notice.
 *
...
object QuireWidgets {
    ...
    const val EXTRA_HABIT_SCREEN = "app.quire.android.extra.HABIT_SCREEN"

    /**
     * Opens Quire.
     *
     * `NEW_TASK` because a broadcast receiver has no task of its own to start an
     * activity in. `SINGLE_TOP` so a reader who already has Quire open gets the
     ...
     * one.
     */
    fun openIntent(context: Context, widget: QuireWidget): Intent =
        Intent(context, MainActivity::class.java)
            .addFlags(Intent.FLAG_ACTIVITY_NEW_TASK or Intent.FLAG_ACTIVITY_SINGLE_TOP)
            .apply { widget.opens?.let { putExtra(EXTRA_HABIT_SCREEN, it.name) } }

    fun pendingOpen(context: Context, widget: QuireWidget): PendingIntent =
        PendingIntent.getActivity(
            context,
            widget.requestCode,
            openIntent(context, widget),
            // Immutable because nothing outside Quire has any business editing it,
            // and required outright from API 31.
            PendingIntent.FLAG_UPDATE_CURRENT or PendingIntent.FLAG_IMMUTABLE,
        )

    /**
     * The destination an intent names, if Quire still has one by that name.
     *
     * `valueOf` would be the obvious read and is the wrong one: a widget pinned by
     * an older version of Quire keeps the intent it was created with for as long as
    ...
    fun refreshIntent(context: Context, widget: QuireWidget, ids: IntArray): Intent =
