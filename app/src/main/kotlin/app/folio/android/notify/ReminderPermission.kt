...
/**
 * When Folio may ask, and when it must stop.
 *
 ...
 *
 * Folio's answer is to ask its own question first — once, in its own words, at the
...
object ReminderPermission {
     ...
     *
     * Only consulted once the reader has said yes inside Folio, so reaching here
     ...
     *
     * The complement of [shouldRequestSystemPrompt] whenever Folio cannot post, so
...
/** The one screen Folio sends the reader to when it is not allowed to ask again. */
object NotificationSettings {

    /**
     * Folio's own notification settings, not the system's top level.
     *
     * Without [Settings.EXTRA_APP_PACKAGE] this opens a screen where the reader has
     * to go hunting for the app they have already told Folio they want notifications
