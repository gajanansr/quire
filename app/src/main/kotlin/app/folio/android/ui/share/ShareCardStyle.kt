...
import app.folio.android.ui.theme.FolioColors
import app.folio.android.ui.theme.FolioPalettes

/**
 * How a share card is dressed.
 *
 * Six looks: the book's own cover colours, and Folio's five palettes. Deliberately
 * the five the app already ships rather than a set invented for this one screen —
 * a share card is the most public thing Folio produces, and it should look like the
...
enum class ShareCardStyle(val label: String) {
    ...
    fun palette(bookId: String?): CardPalette = when (this) {
        ...
        PAPER -> from(FolioPalettes.Paper)
        SEPIA -> from(FolioPalettes.Sepia)
        EINK -> from(FolioPalettes.Eink)
        NIGHT -> from(FolioPalettes.Night)
        BLACK -> from(FolioPalettes.Black)
    }

    private fun from(colors: FolioColors) = CardPalette(
