...
class FolioIconsTest {

    private val app: Application get() = ApplicationProvider.getApplicationContext()

    /**
     * Every `@DrawableRes val` declared on [FolioIcons].
     *
    ...
    private fun declaredIcons(): Map<String, Int> =
        FolioIcons::class.java.declaredFields
            .filter {
            ...
            .associate { field ->
                ...
                field.name to field.getInt(FolioIcons)
            }
    ...
    fun `every icon resolves to a real drawable`() {
        ...
        assertTrue("FolioIcons exposes nothing", icons.isNotEmpty())
        icons.forEach { (name, id) ->
            assertNotNull("FolioIcons.$name does not resolve", app.getDrawable(id))
        }
    }
    ...
    fun `nothing ships that the app cannot name, and nothing named is missing`() {
        ...
        // up (dead weight in the APK), or a name kept in FolioIcons after its
        ...
        assertTrue(
            "these drawables ship but no FolioIcons entry names them: $unused",
            unused.isEmpty(),
        )
        assertTrue(
            "FolioIcons names drawables that are not bundled",
