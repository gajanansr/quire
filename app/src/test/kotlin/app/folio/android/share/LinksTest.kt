...
/**
 * The addresses Folio can send a reader to, and the version it claims to be.
 *
...
class LinksTest {
    ...
    fun `every link that has been filled in is one the system can open`() {
        // A URL without a scheme is not a URL: Intent.ACTION_VIEW on "folio.app" finds
        // no activity, so the row taps and nothing happens — no crash, no message.
        FolioLinks.fillIns.filterValues { it.isNotBlank() }.forEach { (name, link) ->
    ...
    fun `the support link is the author's page and nothing else`() {
        ...
        assertEquals("https://razorpay.me/@gajanansr", FolioLinks.SUPPORT)
        assertEquals(SupportLink.URL, FolioLinks.SUPPORT)
    }
    ...
    fun `blanksIn names every empty link and no filled one`() {
        ...
        assertEquals(listOf("EMPTY", "WHITESPACE"), FolioLinks.blanksIn(links))
    }
    ...
    fun `unset reports what the release checklist still has to supply`() {
        ...
        FolioLinks.unset().forEach { name ->
            assertTrue(
                "unset() named $name, but it has a value",
                FolioLinks.fillIns.getValue(name).isBlank(),
            )
        }
    }
    ...
    fun `a settings row shows the host, not the whole address`() {
        assertEquals("razorpay.me", FolioLinks.displayHost(FolioLinks.SUPPORT))
        assertEquals("folio.app", FolioLinks.displayHost("https://folio.app/privacy"))
        assertEquals("folio.app", FolioLinks.displayHost("https://www.folio.app/"))
        assertEquals("github.com", FolioLinks.displayHost("https://github.com/x/y?tab=z"))
        assertEquals("hello@folio.app", FolioLinks.displayHost("mailto:hello@folio.app"))
        assertEquals("", FolioLinks.displayHost(""))
    }

    @Test
    fun `a mailto carries a findable subject`() {
        assertEquals(
            "mailto:hello@folio.app?subject=Folio%20on%20Android",
            FolioLinks.mailto("hello@folio.app", "Folio on Android"),
        )
    }
    ...
    fun `the version in Settings is the version that was built`() {
        ...
        assertEquals(
            "app/build.gradle.kts no longer declares `val defaultVersionName = \"...\"`; " +
                "FolioRelease.VERSION_NAME can no longer be checked against it",
            true,
            declared != null,
        )
        assertEquals(
            "FolioRelease.VERSION_NAME drifted from the versionName in app/build.gradle.kts",
            declared,
            FolioRelease.VERSION_NAME,
        )
    }
}
