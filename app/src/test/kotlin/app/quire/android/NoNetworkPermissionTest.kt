...
/**
 * Quire's promise, enforced rather than documented.
 *
...
class NoNetworkPermissionTest {
    ...
    @Test
    fun `quire cannot reach the network`() {
        val permissions = requestedPermissions()
        assertFalse(
            "Quire requests INTERNET. A dependency reintroduced network egress; " +
    ...
    fun `no permission grants access to the reader's own data`() {
        ...
        // Quire had gained the ability to read files it was never given.
        ...
        forbidden.forEach { permission ->
            assertFalse("Quire requests $permission", requested.contains(permission))
        }
    }
    ...
    fun `the permissions that remain are local execution only`() {
        ...
        // POST_NOTIFICATIONS is reviewed on the same terms: it lets Quire put a
        ...
        val allowed = setOf(
            ...
            "app.quire.android.DYNAMIC_RECEIVER_NOT_EXPORTED_PERMISSION",
        )
