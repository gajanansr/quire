...
/**
 * The Quire mark, as a widget can wear it.
 *
 * The same fold the launcher draws — one sheet, two leaves — but not the same
 ...
 *    other icon in Quire is, and reads on all five.
 ...
 *
 * Checked by reading the path data back, the way [app.quire.android.ui.theme
...
class QuireMarkTest {
    ...
    private fun leaves(): Pair<List<Pair<Double, Double>>, List<Pair<Double, Double>>> {
        val drawn = paths(R.drawable.quire_mark)
        assertEquals(
    ...
    fun `the mark inflates and is square`() {
        val drawable = app.getDrawable(R.drawable.quire_mark)
