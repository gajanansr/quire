 ...
 *
 * A widget is the one Quire surface nobody can proof-read: it is drawn in the
 ...
 * [app.quire.android.ui.share.ShareCardStyleTest] applies it to the share card,
...
class WidgetPaletteTest {
    ...
    private fun assertContrast(theme: QuireThemeName, what: String, ink: Int, floor: Double) {
    ...
    fun `every theme's secondary text and graphics clear 3 to 1`() {
        QuireThemeName.entries.forEach { theme ->
            ...
            assertContrast(theme, "the Quire mark", palette.mark, 3.0)
        }
    }
    ...
    fun `E-ink stays grey, on the widget as everywhere else`() {
        ...
        val palette = widgetPalette(QuireThemeName.EINK)
        listOf(
    ...
    fun `every theme gets its own card`() {
        ...
        // theme.
        val surfaces = QuireThemeName.entries.map { widgetPalette(it).surface }
        assertEquals(
