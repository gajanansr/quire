...
class StatsWidgetTest {
    ...
    fun `the stats widget is never woken by the clock`() {
        // Nothing here changes while Quire is closed: a book is not finished by the
        // passage of time. A periodic update would spend battery redrawing numbers
        // that cannot have moved. QuireWidgets.refresh is its only refresh.
    ...
    fun `the widget is drawn in the theme the reader chose`() {
        val sepia = widgetPalette(QuireThemeName.SEPIA)
        val root = render(
    ...
    fun `two themes do not draw the same widget`() {
        ...
        val sepia = render(statsWidget(snapshot), QuireThemeName.SEPIA)
        val black = render(statsWidget(snapshot), QuireThemeName.BLACK)
        assertNotEquals(
    ...
    fun `the mark stays on the card when there is nothing else on it`() {
        ...
        assertEquals(
            tint(widgetPalette(QuireThemeName.PAPER).mark),
    ...
    @Test
    fun `the whole widget opens Quire when tapped`() {
