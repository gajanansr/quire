...
class HabitWidgetTest {
    ...
    fun `the habit widget is refreshed by the clock as well as by the app`() {
        ...
        // platform's floor and the right backstop; QuireWidgets.refresh is what
    ...
    fun `a day that was read is accent, a day that was not is the border colour`() {
        ...
        val paper = widgetPalette(QuireThemeName.PAPER)
    ...
    fun `the widget is drawn in the theme the reader chose`() {
        ...
        val root = render(state("7 days", "12 of 20 minutes today"), QuireThemeName.SEPIA)
    ...
    fun `two themes do not draw the same widget`() {
        ...
        // the theme ignored.
        val sepia = render(state("7 days"), QuireThemeName.SEPIA)
        val night = render(state("7 days"), QuireThemeName.NIGHT)
        assertNotEquals(
