...
class ReminderWorkerTest {
    ...
    fun setUp() {
        ...
        db = Room.inMemoryDatabaseBuilder(app, QuireDatabase::class.java)
    ...
    fun `a day the reader has already read is left alone`() = runBlocking {
        ...
        assertTrue("Quire interrupted a day that was already read: ${shade()}", shade().isEmpty())
        assertEquals(
    ...
    fun `a reader with the book open right now is not interrupted`() = runBlocking {
        ...
        assertTrue("Quire interrupted someone mid-chapter: ${shade()}", shade().isEmpty())
        assertEquals(
    ...
    fun `running again the same day says nothing more`() = runBlocking {
        ...
        assertTrue("Quire reminded twice in one day: ${shade()}", shade().isEmpty())
    }
    ...
    fun `switching reminders off leaves nothing behind to wake up later`() = runBlocking {
        ...
        assertTrue("Quire spoke after being switched off: ${shade()}", shade().isEmpty())
        assertTrue(
