...
#
# Shows crashes, warnings and errors from Quire's own process plus any stack
...
PID=$(adb -s "$DEVICE" shell pidof app.quire.android 2>/dev/null | tr -d '\r' || true)
if [ -n "$PID" ]; then
  echo "following app.quire.android (pid $PID) on $DEVICE" >&2
  FILTER=(--pid="$PID")
else
